...
fn sim_counter() -> Result<(), String> {
    ...
    {
        let inst = instances.inst::<Counter<T>>(cntr);
        ...
        assert_eq!(
            instances.inst::<Counter<T>>(cntr).as_t::<T>(q).try_as_u64(),
            Some(32),
            "Value at reset should be the reset value"
        );
    }
    ...
    {
        let mut inst = instances.inst_mut::<Counter<T>>(cntr);
    ...
    *instances.inst_mut::<Counter<T>>(cntr).inputs.decrement = false.into();
    *instances.inst_mut::<Counter<T>>(cntr).inputs.increment = true.into();
    for _ in 0..1_000 {
        sim.fire_next_edges();
    }
    timer.stop();
    *instances.inst_mut::<Counter<T>>(cntr).inputs.increment = false.into();
...
//    BvN<N>: IsBv
{
    ...
    *instances.inst_mut::<Counter<T>>(cntr).inputs.reset_n = true.into();
    for _ in 0..10 {
        sim.fire_next_edges();
    }

    *instances.inst_mut::<Counter<T>>(cntr).inputs.decrement = true.into();
    *instances.inst_mut::<Counter<T>>(cntr).inputs.increment = false.into();
    for _ in 0..1_000 {
        sim.fire_next_edges();
    }
